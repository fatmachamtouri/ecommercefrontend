//payload , action : nom slice/nom de la methode, pending heki lmachya klel backend state.isLoading=true
// +fulfilled recu des donnee stat.articles=action.payload+rejected state.error=action.payload (5atro return )
//pour create article:return res.data=>tetsama action.payload fulfilled l'insertion est effectuée 
import { createSlice, createAsyncThunk } from '@reduxjs/toolkit'
import { addArticle, deleteArticle, editArticle, fetchArticleById, fetchArticles, fetchArticlesPagServ } from "../Service/ArticleService"
...
export const getArticlespages = createAsyncThunk(
    ...
    async (mesparams, thunkAPI) => { //async mata9bel ken deux parametre hak 3lech sna"t mesparams fih deux parametre currentpage et itemsperpage
        const { rejectWithValue } = thunkAPI;
        try {
